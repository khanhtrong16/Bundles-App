...
function optionallyBuildExpandOperation(line): ExpandOperation | null {
  ...
  console.log(`Attempting to expand line ${cartLineId}:`, {
    componentReferences: merchandise.componentReferences,
    componentQuantities: merchandise.componentQuantities,
  });
  ...
  try {
    // Xử lý componentReferences
    if (
      typeof merchandise.componentReferences === "object" &&
      merchandise.componentReferences.value
    ) {
      componentReferences = JSON.parse(merchandise.componentReferences.value);
    } else if (Array.isArray(merchandise.componentReferences)) {
      componentReferences = merchandise.componentReferences;
    } else {
    ...
    // Xử lý componentQuantities
    if (
      typeof merchandise.componentQuantities === "object" &&
      merchandise.componentQuantities.value
    ) {
      componentQuantities = JSON.parse(merchandise.componentQuantities.value);
    } else if (Array.isArray(merchandise.componentQuantities)) {
      componentQuantities = merchandise.componentQuantities;
    } else {
