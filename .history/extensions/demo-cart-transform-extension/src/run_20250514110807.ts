...
export function run(input: { cart: InputCart }): FunctionRunResult {
  console.log("INPUT CART:", JSON.stringify(input.cart, null, 2));
  ...
  console.log("mergeOperations:", JSON.stringify(mergeOperations, null, 2));
  console.log("expandOperations:", JSON.stringify(expandOperations, null, 2));
  console.log(
    "quantityAdjustmentOperations:",
    JSON.stringify(quantityAdjustmentOperations, null, 2),
  );

  // Combine operations and deduplicate
  ...
  console.log("FINAL cartOperations:", JSON.stringify(cartOperations, null, 2));
  return {
...
function getExpandCartOperations(cart: InputCart): CartOperation[] {
  ...
  console.log("CHECKING CART LINES FOR NORMAL EXPAND:");

  return cart.lines.flatMap((line) => {
    const variantMerchandise =
      line.merchandise as InputCartLinesMerchandiseOnProductVariant;

    // Lấy references trước để kiểm tra xem đây có phải là bundle không
    const references = getComponentReferences(variantMerchandise);
    const quantities = getComponentQuantities(variantMerchandise);

    console.log(
      `Line ${line.id} - quantity: ${line.quantity}, has references: ${references.length > 0}`,
    );

    // Kiểm tra xem đây có phải là bundle không
    if (references.length === 0 || references.length !== quantities.length) {
      console.log(`Skipping non-bundle line: ${line.id}`);
      return [];
    }

    // Skip if already processed or quantity > 1 (handled by getQuantityAdjustmentOperations)
    if (processedLineIds.has(line.id)) {
      console.log(`Skipping already processed bundle: ${line.id}`);
      return [];
    }

    // Nếu quantity > 1, sẽ được xử lý bởi getQuantityAdjustmentOperations
    if (line.quantity > 1) {
      console.log(
        `Skipping bundle with quantity > 1: ${line.id}, will be handled by quantity adjustment`,
      );
      return [];
    }

    console.log(`Processing normal bundle expansion: ${line.id}`);
    ...
    console.log(`Created expand operation for bundle: ${line.id}`);
...
function getQuantityAdjustmentOperations(cart: InputCart): CartOperation[] {
  ...
  console.log(
    "CHECKING CART LINES FOR QUANTITY ADJUSTMENT:",
  ...
  for (const line of cart.lines) {
    ...
    if (processedLineIds.has(line.id)) {
      console.log(`Skipping already processed line: ${line.id}`);
      continue;
    }

    const variantMerchandise =
      line.merchandise as InputCartLinesMerchandiseOnProductVariant;
    if (variantMerchandise.__typename !== "ProductVariant") {
      console.log(`Skipping non-variant merchandise: ${line.id}`);
      continue;
    }
    ...
    console.log(`Line ${line.id} has references:`, references);
    console.log(`Line ${line.id} has quantities:`, baseQuantities);
    ...
    if (
      ...
      console.log(
        `Skipping line with invalid references/quantities: ${line.id}`,
      );
      continue;
    }
    ...
    if (line.quantity > 1) {
      console.log(
        `Processing bundle with quantity > 1: ${line.id}, quantity: ${line.quantity}`,
      );
      ...
      console.log(`Created expanded items:`, expandedItems);
      ...
      processedLineIds.add(line.id);
      console.log(`Added quantity adjustment operation for line: ${line.id}`);
    } else {
      console.log(`Skipping line with quantity <= 1: ${line.id}`);
    }
  }

  console.log(`Quantity adjustment operations created: ${operations.length}`);
  return operations;
}
