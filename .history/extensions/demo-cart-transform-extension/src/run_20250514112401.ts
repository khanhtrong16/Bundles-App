...
export function run(input: { cart: InputCart }): FunctionRunResult {
  ...
  console.log("mergeOperations đây:", JSON.stringify(mergeOperations, null, 2));
  console.log(
    "expandOperations đây:",
    JSON.stringify(expandOperations, null, 2),
  );
...
function getMergeCartOperations(cart: InputCart): CartOperation[] {
  ...
  // console.log("variant đây:", JSON.stringify(variant, null, 2));
  for (const definition of mergeParentDefinitions) {
    ...
    // Bỏ qua nếu bundle chỉ có 1 sản phẩm con
    if (definition.component_reference.length === 1) {
      continue;
    }
...
function getExpandCartOperations(cart: InputCart): CartOperation[] {
  ...
  return cart.lines.flatMap((line) => {
    ...
    // Bỏ qua nếu không phải bundle hoặc bundle chỉ có 1 sản phẩm con
    if (
      references.length === 0 ||
      references.length !== quantities.length ||
      references.length === 1
    ) {
      return [];
    }
...
function getQuantityAdjustmentOperations(cart: InputCart): CartOperation[] {
  ...
  for (const line of cart.lines) {
    ...
    if (
      ...
      references.length === 1 // Bỏ qua nếu bundle chỉ có 1 sản phẩm con
    ) {
      continue;
    }
