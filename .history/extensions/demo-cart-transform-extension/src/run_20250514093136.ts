...
export function run(input: { cart: InputCart }): FunctionRunResult {
  const cartOperations: CartOperation[] = [
    ...
    ...getQuantityAdjustmentOperations(input.cart),
  ];
...
// handle quantity adjustments for parent products
function getQuantityAdjustmentOperations(cart: InputCart): CartOperation[] {
  ...
  for (const line of cart.lines) {
    ...
    // If quantity is greater than 1, we need to expand the parent into components
    // with adjusted quantities based on the parent quantity
    if (line.quantity > 1) {
...
function getExpandCartOperations(cart: InputCart): CartOperation[] {
  return cart.lines.flatMap((line) => {
    ...
    // Don't expand here if quantity > 1, since getQuantityAdjustmentOperations will handle it
    if (line.quantity > 1) {
      return [];
    }
