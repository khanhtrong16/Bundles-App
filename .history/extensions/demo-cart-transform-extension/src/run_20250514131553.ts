...
function getMergeCartOperations(cart: InputCart): CartOperation[] {
  ...
  const eligibleLines = cart.lines.filter((line) => {
    ...
    if (
      merchandise.component_quantities &&
      ...
      try {
        ...
        // Chỉ merge nếu mảng chứa đúng 1 phần tử
        return Array.isArray(quantitiesArray) && quantitiesArray.length === 1;
      } catch (e) {
        return false;
      }
    }
  ...
  console.log("Các sản phẩm đủ điều kiện merge:", eligibleLines.length);
  for (const definition of mergeParentDefinitions) {
    if (processedParentVariants.has(definition.id as string)) {
      continue;
    }

    // Kiểm tra xem bundle này có đúng 1 thành phần không
    if (definition.component_quantities.length !== 1) {
      continue; // Bỏ qua nếu không phải bundle 1 thành phần
    }
