...
function getComponentParents(
  ...
  const component_quantities = variant.component_quantities;
  const metafields = component_quantities?.map((item) => {
    let metafields = [];
    if (JSON.parse(item.value).length < 2) {
      metafields = JSON.parse(variant.component_parents.value);
    }
    return metafields;
  });
  console.log("metafields đây:", JSON.stringify(metafields, null, 2));
