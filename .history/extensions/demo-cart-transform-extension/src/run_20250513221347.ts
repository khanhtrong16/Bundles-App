...
function getMergeCartOperations(cartLines): CartOperation[] {
  ...
  for (const def of mergeDefs) {
    ...
    if (matchingLines.length === def.component_reference.length) {
      ...
      if (def.price_adjustment) {
        try {
          price = {
            percentageDecrease: {
              value:
                typeof def.price_adjustment === "string"
                  ? parseFloat(def.price_adjustment)
                  : def.price_adjustment,
            },
          };
        } catch (error) {
          console.error(`Error parsing price adjustment for ${def.id}:`, error);
          price = undefined;
        }
      }
    ...
    if (componentParents) {
      ...
      try {
        ...
        for (const entry of parsed) {
          ...
          // Xử lý component_reference
          if (
            typeof entry.component_reference === "object" &&
            entry.component_reference?.value
          ) {
            // Có thể là chuỗi JSON cần parse
            try {
              component_reference = JSON.parse(entry.component_reference.value);
            } catch (e) {
              console.error(`Error parsing component_reference.value:`, e);
          ...
          if (
            ...
            // Có thể là chuỗi JSON cần parse
            try {
              component_quantities = JSON.parse(
                entry.component_quantities.value,
              );
            } catch (e) {
              console.error(`Error parsing component_quantities.value:`, e);
...
function getComponentsInCart(cartLines, def): CartLineInput[] {
  ...
  console.log(`Looking for components for parent ID: ${def.id}`);
  console.log(`Component references:`, def.component_reference);
  console.log(`Component quantities:`, def.component_quantities);
  ...
  for (let i = 0; i < def.component_reference.length; i++) {
    ...
    if (matchingLine) {
      ...
      lineTracker.set(matchingLine.id, newQty);
      console.log(
        `Updated quantity for ${matchingLine.id}: ${currentQty} -> ${newQty}`,
      );
    } else {
      console.log(`No matching line found for component ${refId}`);
      // Nếu không tìm thấy line phù hợp, trả về kết quả rỗng ngay lập tức
      return [];
    }
  }
