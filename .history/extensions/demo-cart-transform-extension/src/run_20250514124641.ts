...
function getMergeCartOperations(cart: InputCart): CartOperation[] {
  ...
  console.log(
    ...
    JSON.stringify(JSON.parse(component_quantities[0].value).length, null, 2),
  );
  if (JSON.parse(component_quantities[0].value).length > 1) {
    return [];
  }
  ...
  for (const definition of mergeParentDefinitions) {
    ...
    // console.log("definition đây:", JSON.stringify(definition, null, 2));
