...
function getExpandCartOperations(cart: InputCart): CartOperation[] {
  ...
  return cart.lines.flatMap((line) => {
    ...
    if (processedLineIds.has(line.id) || line.quantity > 1) {
      return [];
    }
    ...
    console.log("references đây:", JSON.stringify(references, null, 2));
    console.log("quantities đây:", JSON.stringify(quantities, null, 2));
...
function getComponentReferences(
  ...
  console.log(
    "variant.component_reference đây:",
    JSON.stringify(variant.component_reference, null, 2),
  );
